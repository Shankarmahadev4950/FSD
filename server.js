...
fastify.patch('/api/exchanges/:id/accept', { preHandler: authenticate }, async (request, reply) => {
    try {
        ...
        if (exchange.provider._id.toString() !== request.currentUser._id.toString()) {
            return reply.status(403).send({ error: 'Only the provider can accept exchange requests' });
        }
        
        if (exchange.status !== 'pending') {
            return reply.status(400).send({ error: 'Exchange is not in pending status' });
        }
        ...
        // Notify learner via WebSocket if available
        const learnerWs = activeConnections.get(exchange.learner._id.toString());
...
fastify.patch('/api/exchanges/:id/reject', { preHandler: authenticate }, async (request, reply) => {
    try {
        ...
        // Check if user is the provider
        if (exchange.provider._id.toString() !== request.currentUser._id.toString()) {
            return reply.status(403).send({ error: 'Only the provider can reject exchange requests' });
        }
        ...
        exchange.respondedAt = new Date();
        exchange.rejectionReason = request.body.reason || 'No reason provided';
        await exchange.save();
...
fastify.patch('/api/exchanges/:id/cancel', { preHandler: authenticate }, async (request, reply) => {
    try {
        ...
        // Check if user is the learner who made the request
        if (exchange.learner.toString() !== request.currentUser._id.toString()) {
            return reply.status(403).send({ error: 'Only the learner can cancel this exchange request' });
        }
        
        if (exchange.status !== 'pending') {
            return reply.status(400).send({ error: 'Only pending exchanges can be cancelled' });
        }
